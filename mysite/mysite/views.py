...
employeeShiftQuery = (
    "SELECT D{} from kara.dbo.empshift where emp_no={} and year={} and month={}"
)
...
def test(request):
    ...
    for index in range(0, len(allDataFile)):
        year, month, day = jalali.Gregorian(
            allDataFile[index][1].strftime("%Y-%m-%d")
        ).persian_tuple()
        employee_shift = cursor.execute(
            employeeShiftQuery.format(day, allDataFile[index][0], year, month)
        )
        employee_shift = employee_shift.fetchall()
        if len(employee_shift) != 0:  # agar 0 bood ina doctoranb felanan
            freeOrPay = cursor.execute(
                freeOrPayQuery.format(
                    "term{}".format(allDataFile[index][2]), employee_shift[0][0]
                )
            )
            freeOrPay = freeOrPay.fetchall()
            if len(freeOrPay) != 0:
                mealCode = cursor.execute(
                    mealCodeQuery.format(allDataFile[index][1], allDataFile[index][2])
                )
                mealCode = mealCode.fetchall()
                mealPrice = cursor.execute(
                    mealPriceQuery.format(mealCode[0][0], allDataFile[index][1])
                )
                mealPrice = mealPrice.fetchall()
                mealEmpPrice = cursor.execute(
                    mealEmpPriceQuery.format(mealCode[0][0], allDataFile[index][1])
                )
                ...
                # else:
                calculatedPrice = (
                    mealEmpPrice[0][0] if freeOrPay[0][0] else mealPrice[0][0]
                ) + ((allDataFile[index][3] - 1) * mealPrice[0][0])
                meal_name = cursor.execute(
                    "SELECT title FROM meals where meal_code={}".format(mealCode[0][0])
                )
                meal_name = meal_name.fetchall()
                emp_name = cursor.execute(
                    "SELECT name,family,emp_type FROM kara.dbo.employee where emp_no={}".format(
                        int(allDataFile[index][0])
                    )
                )
